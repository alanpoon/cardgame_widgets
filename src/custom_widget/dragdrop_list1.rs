...
impl Items {
    ...
    pub fn next<T>(&mut self, ui: &Ui) -> Option<Item>
        where T: Clone + Send + 'static + Debug
    {
        ...
        let node_index =
            match ui.widget_graph()
                      .widget(list_id)
                      .and_then(|container| {
                                    container.unique_widget_state::<DragDropList<T>>()
                                })
    ...
    where T: Clone + Send + 'a + 'static + Debug
{
    ...
    fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
        ...
        println!("update value_C{:?} items{:?}",
                 items.next::<T>(ui),
                 state.ids.items.len());
        if let Some(_) = state.last_release {
            println!("last_release");
            while let (Some(item), Some(k_h)) = (items.next::<T>(ui), values_c_iter.next()) {
                println!("c {}", c);
